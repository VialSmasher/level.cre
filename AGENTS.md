...
- After a confirmed send, run `scripts/codex/record-levelcre-sales-activity.ps1` with contact, company, email, subject, timestamp, disposition, and a short note. The installed `outlook-sales-followup` skill contains the same recorder for tasks outside this repository. Do not send the full message body to the activity bridge.
