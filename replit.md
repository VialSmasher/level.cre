...
- The Postmark webhook accepts the shared inbound secret through `?secret=...`, `Authorization: Bearer ...`, `x-levelcre-inbound-secret`, or Basic auth where the username or password equals the secret. If captures stop, check Postmark webhook delivery history for 401s or an old webhook URL before changing parsing code.
