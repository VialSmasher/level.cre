...
- The Inbox refresh path also has a lightweight Outlook BCC recovery sync at `POST /api/email/outlook/sync-bcc`. It checks recent Sent Items for messages BCC'd to the configured Level CRE intake address and creates `needs_context` review rows. Keep this BCC-only fallback small so it does not flood the inbox with every Outlook sent/inbox message.
