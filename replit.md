...
- When `EMAIL_INBOUND_ADDRESS` / `INBOUND_EMAIL_ADDRESS` is a fixed Postmark address such as `{hash}@inbound.postmarkapp.com`, that hash may not be a Level CRE user id. In that case, the inbound resolver should verify candidate ids against `public.users`, then fall back to matching the email sender against `public.users.email`, `public.profiles.email`, and `email_connections.email_address`, then fall back to the sole Level CRE user only when there is exactly one user. Outlook connection is optional recovery, not a requirement for Postmark BCC capture.
...
- The Inbox refresh path also has a lightweight Outlook BCC recovery sync at `POST /api/email/outlook/sync-bcc`. The existing `GET /api/email/review` route also runs this small recovery before returning rows so an older deployed frontend can still recover recent BCC captures. It checks recent Sent Items for messages BCC'd to the configured Level CRE intake address and creates `needs_context` review rows. Keep this BCC-only fallback small so it does not flood the inbox with every Outlook sent/inbox message.
- The primary Postmark BCC path must not require a Level CRE Outlook login. The webhook accepts the shared inbound secret through `?secret=...`, `Authorization: Bearer ...`, `x-levelcre-inbound-secret`, or Basic auth where the username or password equals the secret, but it should also accept a Postmark payload addressed to the configured Level CRE intake address. If captures stop, check Postmark webhook delivery history for 401s, an old webhook URL, or missing recipient fields before changing parsing code.
