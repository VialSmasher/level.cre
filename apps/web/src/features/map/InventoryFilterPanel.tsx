...
import { INVENTORY_SIGNAL_GROUPS, defaultInventoryFilters, propertyFilterFacts, matchesPropertyFilters, type InventoryFilters } from './inventoryFilters'

export function InventoryFilterPanel({prospects, filters, onChange, onFocus}: {
  prospects: Array<{aiMetadata?: unknown}>; filters: InventoryFilters; onChange: (filters: InventoryFilters)=>void; onFocus:(filters:InventoryFilters)=>void
}) {
  const inventories=useMemo(()=>prospects.map(propertyFilterFacts).filter(row=>row.classification!=='unknown'),[prospects])
  ...
  const selected=prospects.filter(prospect=>propertyFilterFacts(prospect).classification!=='unknown'&&matchesPropertyFilters(prospect,filters)).length
  ...
  return <section className="border-t border-slate-200 py-3" aria-label="Property inventory filters">
    ...
    <div className="mt-2 grid gap-2">
      ...
      <button type="button" onClick={()=>{const next={...defaultInventoryFilters(),includeOther:false,tags:['long_hold']};onChange(next);onFocus(next)}} className="min-h-10 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-left text-xs font-semibold text-amber-950">15+ year registrations <span className="float-right tabular-nums">{counts.get('long_hold')||0}</span></button>
      <div className="grid grid-cols-2 gap-2"><button type="button" onClick={()=>preset(false)} className="min-h-9 rounded-md border px-2 text-xs font-medium">All classified assets</button><button type="button" onClick={()=>onFocus(filters)} className="min-h-9 rounded-md border px-2 text-xs font-medium">Fit selected assets</button></div>
    </div>
    ...
    <label className="mt-2 flex min-h-9 cursor-pointer items-center gap-2 text-xs"><input type="checkbox" checked={filters.includeOther} onChange={event=>onChange({...filters,includeOther:event.target.checked})}/>Show unclassified / other records</label>
