...
export const intelListings = pgTable(
  "intel_listings",
  {
    ...
    geocodeStatus: varchar("geocode_status").notNull().default("pending"),
    geocodeConfidence: numeric("geocode_confidence", { precision: 5, scale: 2 }),
    geocodeSource: varchar("geocode_source"),
    dataQualityStatus: varchar("data_quality_status").notNull().default("review"),
  ...
  (table) => [
    ...
    index("IDX_intel_listings_geocode_status").on(table.geocodeStatus),
    index("IDX_intel_listings_data_quality_status").on(table.dataQualityStatus),
