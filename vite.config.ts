...
export default defineConfig(async ({ mode }) => {
  ...
  const GOOGLE_ENABLED = (env.VITE_ENABLE_GOOGLE_AUTH === '1' || env.VITE_ENABLE_GOOGLE_AUTH === 'true');
  ...
  return {
    ...
    // Expose the server-style Google Maps key for hosted dev tools that cannot
